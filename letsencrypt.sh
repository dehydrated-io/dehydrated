...
sign_domain() {
  ...
  if [ -n "${HOOK_POST_CREATION}" ]; then
      ${HOOK_POST_CREATION} "${WELLKNOWN}/${challenge_token}" "${keyauth}" "${BASEDIR}/certs/${domain}/fullchain.pem" "${BASEDIR}/certs/${domain}/privkey.pem" "${BASEDIR}/certs/${domain}/cert.pem" 
  fi
